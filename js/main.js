...
document.addEventListener("DOMContentLoaded", function () {
  ...
  tabWrappers.forEach((tabWrapper) => {
    const leftButtons = tabWrapper.parentNode.querySelectorAll(
      ".nav-tabs-scroll.left"
    );
    const rightButtons = tabWrapper.parentNode.querySelectorAll(
      ".nav-tabs-scroll.right"
    );

    // wheel 이벤트에 passive 옵션 적용
    tabWrapper.addEventListener(
      "wheel",
      function (e) {
        if (e.deltaY > 0) {
          tabWrapper.scrollLeft += 400;
        } else {
          tabWrapper.scrollLeft -= 400;
        }
      },
    ...
    // 스크롤 및 리사이즈 이벤트에 updateButtonState 적용
...
$(document).ready(function () {
  ...
  $("#chkAll").on("click", function () {
    ...
    if ($("#chkAll").is(":checked")) {
      ...
      $("#chkAll").removeClass("input-chk-all-relis");
    } else {
  ...
  $("input[name=chk]").on("click", function () {
    ...
    const chkChked = $("input[name=chk]:checked").length;
    ...
    if (chkChked === 0) {
      ...
      // 개별 체크박스 중 하나라도 선택 해제된 경우
      $("#chkAll").prop("checked", false);
      $("#chkAll").addClass("input-chk-all-relis");
    } else {
      // 모두 선택된 경우
...
// .chip-del 삭제 이벤트
document.querySelectorAll(".chip-del").forEach(function (deleteButton) {
...
// .chip-more chip-list 더보기 이벤트
$(document).ready(function () {
  $(".chip-more").on("click", function () {
    // .active 클래스 토글
    $(".chip-more").toggleClass("active");

    // .chip-more.active이면 max-height: 100% 적용, 아니면 초기화
    if ($(".chip-more").hasClass("active")) {
      $(".chip-list").addClass("expanded");
    } else {
      $(".chip-list").removeClass("expanded");
    }
  });
});
